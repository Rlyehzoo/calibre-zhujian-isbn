...
def to_metadata(browser, log, entry_, timeout):  # {{{
    ...
    tags = entry_.get("labels")

    publisher = None
    pubdate = None
    authors = None

    if "abstract" in entry_:
        book_info = entry_["abstract"]
        info = book_info.split('/')
        authors = '/'.join(info[0:-3])
        pubdate = info[-2]
        publisher = info[-3]
    elif "book_info" in entry_:
...
if __name__ == '__main__':  # tests {{{
    ...
    tests = [
        #(
        #       {
        #           "identifiers": {"isbn": " 9787020120130"},
        #           "title": "我的天才女友",
        #           "authors": ["埃莱娜·费兰特"],
        #       },
        #       [title_test("我的天才女友", exact=True), authors_test(["埃莱娜·费兰特"])],
        #   ),
            (
                {"title": "档案：一部个人史"},
                [title_test("档案：一部个人史", exact=False)],
            ),
    ]
